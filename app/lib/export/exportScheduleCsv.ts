...
export function scheduleRowsToCsv(rows: ScheduleRow[]): string {
  const headers = ["Month", "EMI", "Principal Paid", "Interest Paid", "Prepayment", "Balance"];
...
export function exportScheduleCsv(rows: ScheduleRow[], filename?: string): void {
  if (typeof window === "undefined" || rows.length === 0) return;

  const csv = scheduleRowsToCsv(rows);
